...
var tr = class extends R {
	constructor() {
		super(...arguments), this.qt = null, this.MS = [];
	}
	ht(t) {
		this.qt = t, this.MS = [];
	}
	et({ context: t, horizontalPixelRatio: i, verticalPixelRatio: n }) {
		...
		this.MS.length || this.gS(i);
		const s = Math.max(1, Math.floor(n)), e = Math.round(this.qt.bS * n) - Math.floor(s / 2), r = e + s;
		for (let i = this.qt.lt.from; i < this.qt.lt.to; i++) {
			const h = this.qt.ot[i], a = this.MS[i - this.qt.lt.from], l = Math.round(h.ut * n);
			let o, _;
			t.fillStyle = h.sh, l <= e ? (o = l, _ = r) : (o = e, _ = l - Math.floor(s / 2) + s), t.fillRect(a.Na, o, a.bi - a.Na + 1, _ - o);
		}
	}
	gS(t) {
		if (null === this.qt || 0 === this.qt.ot.length || null === this.qt.lt) return void (this.MS = []);
		const i = Math.ceil(this.qt.ml * t) <= 1 ? 0 : Math.max(1, Math.floor(t)), n = Math.round(this.qt.ml * t) - i;
		this.MS = new Array(this.qt.lt.to - this.qt.lt.from);
		for (let i = this.qt.lt.from; i < this.qt.lt.to; i++) {
			const s = this.qt.ot[i], e = Math.round(s._t * t);
			let r, h;
			if (n % 2) {
				const t = (n - 1) / 2;
				r = e - t, h = e + t;
		...
		for (let t = this.qt.lt.from + 1; t < this.qt.lt.to; t++) {
			const n = this.MS[t - this.qt.lt.from], s = this.MS[t - this.qt.lt.from - 1];
			n.wt === s.wt + 1 && n.Na - s.bi !== i + 1 && (s.SS > s.ce ? s.bi = n.Na - i - 1 : n.Na = s.bi + i + 1);
		}
		let s = Math.ceil(this.qt.ml * t);
		for (let t = this.qt.lt.from; t < this.qt.lt.to; t++) {
			const i = this.MS[t - this.qt.lt.from];
			i.bi < i.Na && (i.bi = i.Na);
			const n = i.bi - i.Na + 1;
			s = Math.min(n, s);
		}
		if (i > 0 && s < 4) for (let t = this.qt.lt.from; t < this.qt.lt.to; t++) {
			const i = this.MS[t - this.qt.lt.from];
			i.bi - i.Na + 1 > s && (i.SS > i.ce ? i.bi -= 1 : i.Na += 1);
		}
	}
};
var ir = class extends Se {
	constructor() {
		super(...arguments), this.Vg = new tr();
	}
	Bg(t, i) {
		const n = this.ae.Ft().Nt(this.ae.N().base, u(this.ae.Lt()).Wt);
		return null === n ? null : Ps(this.kg, this.Tg, t, i, this.le.Et().ml(), this.ae.N().hitTestTolerance, ((t, i) => {
			i[0] = t.ut, i[1] = n;
		}));
	}
	Rb(t, i, n) {
	...
	Ng() {
		const t = {
			...
			ml: this.le.Et().ml(),
			lt: this.Tg,
			bS: this.ae.Ft().Nt(this.ae.N().base, u(this.ae.Lt()).Wt)
		};
		this.Vg.ht(t);
	}
};
...
({ ...e });
//#endregion
export { ye as a, nr as i, Qe as n, le as r, $i as t };
